...
public class VelocityGameManager implements GameManager, AutoCloseable {
    ...
    public CompletableFuture<GameServer> getGameServer(@NonNull String name) {
        ...
        if (server == null) {
            ...
            return server.ping().thenApply(ping -> {
                int maxPlayers = 0;
                int playerCount = 0;
                String gameMode = name.substring(0, name.lastIndexOf("-"));
                ...
                return new CloudGameServer(name, maxPlayers, playerCount, gameMode);
            });
        }
    }
    ...
    public CompletableFuture<List<GameServer>> getGameServersByGameMode(String gameMode) {
        return CompletableFuture.supplyAsync(() -> {
            Collection<RegisteredServer> servers = cloud.getProxyServer().getAllServers();
            CompletableFuture[] gameServers = new CompletableFuture[servers.size()];

            AtomicInteger i = new AtomicInteger();
            servers.forEach(server -> {
                String name = server.getServerInfo().getName();
                if (gameMode != null && !name.startsWith(gameMode + "-")) {
                    return;
                }

                gameServers[i.getAndIncrement()] = getGameServer(name);
            });
